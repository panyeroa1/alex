...
const BiometricsEnrollment: React.FC<{ onEnrollmentComplete: () => void; }> = ({ onEnrollmentComplete }) => {
    ...
    // FIX: `useRef<number>()` requires an initial value. Changed to `useRef<number | null>(null)` to correctly initialize the ref that will hold the timer ID.
            ...
            case 'idle':
                return (
                    <>
                        ...
                        <button onClick={startRecording} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full transition-all duration-300 transform hover:scale-105">
                            Start Enrollment
                        </button>
                    </>
                );
    ...
    return (
        <div className="absolute inset-0 bg-black z-50 flex flex-col items-center justify-center text-center p-4">
             <style>{`.animate-progress { animation: progress-bar 5s linear forwards; } @keyframes progress-bar { from { width: 0% } to { width: 100% } }`}</style>
            {getStatusContent()}
        </div>
    );
};
        ...
        <div className="absolute inset-0 z-30 bg-black flex flex-col p-4 font-sans">
            <header className="flex items-center justify-between pb-4 border-b border-white/10">
                <div className="flex items-center gap-2">
                    <button onClick={onOpenSidebar} className="p-2 rounded-full hover:bg-white/10" aria-label="Open Conversation History">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg>
                    </button>
                    <h1 className="text-xl font-bold">Alex</h1>
                </div>
                <button onClick={onSwitchToVoice} className="p-2 rounded-full hover:bg-white/10" aria-label="Switch to Voice Mode">
            ...
            <footer className="flex items-center gap-2 pt-4 border-t border-white/10">
                 ...
                 <button onClick={() => fileInputRef.current?.click()} className="p-3 rounded-full hover:bg-white/10 transition-colors" aria-label="Attach File">
                ...
                />
                <button onClick={handleSend} className="p-3 rounded-full bg-blue-600 hover:bg-blue-700 transition-colors" aria-label="Send Message">
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            if (convos.length > 0) {
                ...
                handleNewConversation(false); // Create first conversation without closing sidebar
            }
        };
    ...
    const handleToolCall = useCallback(async (fc: FunctionCall, source: 'voice' | 'chat') => {
        ...
        if (result.startsWith('Error')) addNotification(result, 'error'); else addNotification(result, 'success');
    ...
    const startSession = useCallback(async () => {
        try {
            ...
            const { session, audioStream, toggleOutputMute, outputAudioContext, outputGainNode } = await connectToLiveSession({
    ...
    }, [addNotification, handleToolCall, updateTranscript, endSession, isVideoEnabled, transcript]);
    ...
    const handleSwitchToChat = async () => {
        await endSession();
        chatRef.current = await startChatSession(transcript);
        setView('chat');
    ...
    const handleSendTextMessage = async (message: string) => {
        if (!chatRef.current) { // Ensure chat session exists
             chatRef.current = await startChatSession(transcript);
        }
        ...
        <div className="relative flex flex-col h-screen bg-black text-white font-sans overflow-hidden">
            ...
            <div className={`transition-opacity duration-500 ${view === 'chat' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                ...
                <main className="flex-1 flex items-center justify-center relative">
                    ...
                    {showCc && (
                        <div className="absolute bottom-24 left-4 right-4 z-20 h-48 bg-black/30 backdrop-blur-sm rounded-lg p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
                            {transcript.map((msg) => (
                ...
                <footer className="absolute bottom-0 left-0 right-0 p-6 z-10">
                    <div className="flex justify-around items-center max-w-sm mx-auto">
                        <button onClick={handleToggleVideo} className={`w-14 h-14 flex items-center justify-center rounded-full text-white transition-colors ${isVideoEnabled ? 'bg-blue-500/50 hover:bg-blue-500/70' : 'bg-white/10 hover:bg-white/20'}`} aria-label={isVideoEnabled ? "Turn off Video" : "Turn on Video"}>
                        ...
                        <button onClick={handleToggleScreenShare} className={`w-14 h-14 flex items-center justify-center rounded-full text-white transition-colors disabled:opacity-30 disabled:pointer-events-none ${isScreenSharing ? 'bg-blue-500/50 hover:bg-blue-500/70' : 'bg-white/10 hover:bg-white/20'}`} aria-label={isScreenSharing ? "Stop Sharing Screen" : "Share Screen"}>
                        ...
                        <button onClick={handleToggleRecording} disabled={agentStatus === 'idle' || agentStatus === 'connecting' || agentStatus === 'verifying'} className={`w-14 h-14 flex items-center justify-center rounded-full text-white transition-colors disabled:opacity-30 disabled:pointer-events-none ${isRecording ? 'bg-red-600/80 animate-pulse' : 'bg-white/10 hover:bg-white/20'}`} aria-label={isRecording ? "Stop Recording" : "Start Recording"}>
                        ...
                        <button onClick={handleSwitchToChat} className="w-14 h-14 flex items-center justify-center rounded-full bg-white/10 text-white/70 hover:bg-white/20 transition-colors" aria-label="Switch to Chat Mode">
